    ...
    if (face_array.count > 0) 
    {
      for (int i = 0; i < face_array.count; i++) 
      {
        ...
        */
        ...
        // 可以采用opencv来绘制矩形框,颜色格式是B、G、R
        cv::rectangle(show_img,cv::Point(left, top),cv::Point(right, bottom),cv::Scalar(0,0,255),3,8,0);
        show_img.release();
        */
        /*
          // input_image是彩色图（rgb888格式），可以使用rockx封装好的API绘制矩形框
          rockx_point_t pt1={left,top};
          rockx_point_t pt2={right,bottom};
          rockx_color_t color={0,0,255};// 颜色格式是B、G、R
          rockx_image_draw_rect(&input_image, pt1, pt2, color, 3);
          */
      }
    }
  }
...
void *rkmedia_vi_rockx_thread111(void *args) 
{
  ...
  // if(landmark_count == 68)
  // {
  //   ret = rockx_create(&face_landmark_handle,ROCKX_MODULE_FACE_LANDMARK_68, config, sizeof(rockx_config_t));
  //   if (ret != ROCKX_RET_SUCCESS) 
  //   {
  //     printf("init rockx module ROCKX_MODULE_FACE_LANDMARK_68 error %d\n",ret);
  //   }
  ...
  // if(landmark_count == 5)
  // {
  //   ret = rockx_create(&face_landmark_handle,ROCKX_MODULE_FACE_LANDMARK_5, config, sizeof(rockx_config_t));
  //   if (ret != ROCKX_RET_SUCCESS) 
  //   {
  //     printf("init rockx module ROCKX_MODULE_FACE_LANDMARK_5 error %d\n",ret);
  //   }
